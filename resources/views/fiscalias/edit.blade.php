...
<div class="card max-800">
   ...
   {{--}} <a href="{{ route('fiscalias.index') }}"> Ver listado de Fiscalias</a>--}}
    ...
    <form action="{{ route('fiscalias.update', $t_fiscalias->id) }}" method ="POST" enctype="multipart/form-data" class="row g-3 form-register">
        @csrf
        {{ method_field('PUT') }}
   
        <div class="col-md-4">
        <label for="f1" class="form-label">Departamento:</label>
        <input type="text" name="departamento" class="form-control" value="{{ $t_fiscalias->departamento }}" id="f1" required>
        </div>
        <div class="col-md-4">
        <label for="f1" class="form-label">Denominacion:</label>
        <input type="text" name="denominacion" class="form-control" value="{{ $t_fiscalias->denominacion }}" id="f2" required>
        </div>
